...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        rentals_due_soon = BookRental.objects.filter(
            due_date__lte=two_days_from_now, returned=False
        )
        for rental in rentals_due_soon:
            ...
            from_email = "noreply@onlinelibrary.com"
            recipient_list = [rental.user.email]
            send_mail(subject, message, from_email, recipient_list, fail_silently=False)


# You can also implement the logic for allowing extensions or automatically
